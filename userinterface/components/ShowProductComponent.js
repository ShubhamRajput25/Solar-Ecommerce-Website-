...
export default function ShowProductComponent(props){
 ...
 const handleNextPage=(item)=>{
  ...
  var save=parseInt(item.price)-parseInt(item.offerprice)
    ...
    return(<div style={{width:matches3?'100%':"230px",height:matches3?'':'330px',boxShadow:'1px 1px 10px #dfe4ea',display:'flex',flexDirection:matches3?'row':'column',justifyContent:'start',background:'#fff',alignItems:'center',marginTop:'1%',overflow:'hidden',marginLeft:'.6%'}} >
...
<div style={{width:'100%'}}>
    ...
    <div style={{fontWeight:'bolder',marginTop:'10px',display:'flex',justifyContent:'center',alignItems:'center',flexDirection:'column'}}>
        <div>{item.offerprice} <del style={{color:'grey'}}> {item.offerprice}</del> </div>
